...
impl MachineStateLogger {
    ...
    fn insert_machine_state(&mut self, snapshot: &MachineStateSnapshot) -> Result<()> {
        ...
        ]).context("Failed to insert machine_state")?;
        info!(target: "machine_state_logger", "✓ Inserted machine_state: id={}", snapshot.state_id);
        Ok(())
    }
            ...
            if let Some(tx) = guard.as_ref() {
                match tx.try_send(DbWriteCommand::InsertMachineState(snapshot.clone())) {
                    ...
                    Err(std::sync::mpsc::TrySendError::Full(_)) => {
                        warn!(target: "machine_state_logger", "DB write buffer full - dropping snapshot");
                    }
                    Err(_) => {},
                }
            }
        ...
        if let Ok(guard) = self.write_tx.lock() {
            if let Some(tx) = guard.as_ref() {
                match tx.try_send(DbWriteCommand::InsertOperation(event.clone())) {
                    Ok(_) => {},
                    Err(std::sync::mpsc::TrySendError::Full(_)) => {
                        warn!(target: "machine_state_logger", "DB write buffer full - dropping operation");
                    }
                    Err(_) => {},
                }
            }
